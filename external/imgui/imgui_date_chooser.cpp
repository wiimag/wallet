...
namespace ImGui {
    ...
    bool DateChooser(const char* label, tm& dateOut, const char* dateFormat,
        bool closeWhenMouseLeavesIt, bool* pSetStartDateToDateOutThisFrame,
        ...
        const char* downArrowString)
    {
        ...
        if (strlen(dayNames[0]) == 0) {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (strlen(dayName) == 0) {
                    static const char* fallbacks[7] = { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
                    strcpy(dayName, fallbacks[i]);
                }

                const char* dayNameTranslation = tr_cstr(dayNames[i], strlen(dayNames[i]));
                if (dayNameTranslation) {
                    strcpy(dayNames[i], dayNameTranslation);
                }
            ...
            for (int i = 0; i < 12; i++) {
                ...
                const char* monthNameTranslation = tr_cstr(monthNames[i], strlen(monthNames[i]));
                if (monthNameTranslation) {
                    strcpy(monthNames[i], monthNameTranslation);
                }
            }
        }
